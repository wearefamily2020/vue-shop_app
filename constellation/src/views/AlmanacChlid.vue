  ...
  <div class="box">
    <mt-navbar>
      <div
        ...
        @click="getdata(consName,(type = item.src), index)"
      >{{ item.name }}</div>
    </mt-navbar>
    <main>
      <h1>{{ dateList.name }}</h1>
      ...
      <div v-if="dateList.all">
        综合指数:
        <span>{{ dateList.all }}</span>
      </div>
      <div v-if="dateList.color">
        幸运色:
        <span>{{ dateList.color }}</span>
      </div>
      <div>
        健康指数:
        <span>{{ dateList.health }}</span>
      </div>
      <div>
        爱情指数:
        <span>{{ dateList.love }}</span>
      </div>
      <div v-if="dateList.money">
        财运指数:
        <span>{{ dateList.money }}</span>
      </div>
      <div v-if="dateList.number">
        幸运指数:
        <span>{{ dateList.number }}</span>
      </div>
      <div v-if="dateList.QFriend">
        速配星座:
        <span>{{ dateList.QFriend }}</span>
      </div>
      <div v-if="dateList.work">
        工作指数:
        <span>{{ dateList.work }}</span>
      </div>
      <div v-if="dateList.summary">
        今日描述:
        <span>{{ dateList.summary }}</span>
      </div>
    </main>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    this.getdata(this.consName, this.type);
    this.$indicator.open({
      text: "加载中"
    });
  },
  ...
  methods: {
    ...
    getdata(consName, type, index = 0) {
      // console.log(type, index);
      this.iscur = index;
      this.type = type;
      this.api(consName, type);
    }
  }
};
</script>
